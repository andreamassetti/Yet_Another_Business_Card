...
#define PADDLE_Y (MATRIX_ROWS - 2) // Paddle is two rows from the bottom
#define BRICK_ROWS 3

#define SCORE_AREA_ROWS 2 // <-- NEW: Rows 0 and 1 are for score/buffer
#define BRICK_START_ROW SCORE_AREA_ROWS // <-- NEW: Bricks start at row 2
...
static int ball_vx, ball_vy; // ball velocity
static int paddle_x;
static int paddle_vx = 1; // Paddle velocity (1 or -1)
static uint8_t button_pressed_last_frame = 0; // For toggle logic
static uint8_t game_over = 0;
static uint32_t score = 0; // <-- MODIFIED: This will now be points (bricks * 5)
...
void game_init(void)
{
    ...
    paddle_vx = 1; // Start moving right
    button_pressed_last_frame = 0; // Clear button state

    // Place ball just above the paddle
    ball_y = PADDLE_Y - 1; // This is now (MATRIX_ROWS - 3)
...
void update_game_state(uint32_t tick)
{
    ...
    if (game_over)
    {
        uint8_t button_now = read_button();
        // Check for new press to restart
        if (button_now && !button_pressed_last_frame)
        {
            game_init();
        }
        button_pressed_last_frame = button_now;
        draw_game_over_screen(tick); // Show flashing 'G'
        return;
    }

    // 3. Update Paddle Position (Toggle Logic)
    uint8_t button_now = read_button();

    // Check for a new press (rising edge: was 0, is now 1)
    if (button_now && !button_pressed_last_frame)
    {
        paddle_vx = -paddle_vx; // Toggle direction
    }
    button_pressed_last_frame = button_now;
    ...
    {
    	paddle_x = 0;
    	paddle_vx = -paddle_vx; // Bounce off the wall
    }
    ...
    //  b. Top Wall (now bounces off row 1)
    if (ball_y <= (BRICK_START_ROW - 1)) // <-- MODIFIED
    {
        ball_y = (BRICK_START_ROW - 1);
        ball_vy = -ball_vy;
    }

    //  c. Floor (Game Over)
    if (ball_y >= (MATRIX_ROWS - 1)) // This is row 19
    {
        game_over = 1;
        return;
    }
    ...
    //  e. Bricks (now checks in the new position)
    if (ball_y >= BRICK_START_ROW && ball_y < (BRICK_START_ROW + BRICK_ROWS)) // <-- MODIFIED
    {
        int brick_row = ball_y - BRICK_START_ROW; // <-- NEW
        if (bricks[brick_row][ball_x] == 1)
        {
            bricks[brick_row][ball_x] = 0; // Break brick
            ball_vy = -ball_vy;            // Bounce
            score += 5; // <-- MODIFIED: Add 5 points
        }
    }
...
void draw_game_to_framebuffer(void)
{
    ...
    // 2. Draw the score bar
    draw_score();

    // 3. Draw bricks (in their new position)
    for (int r = 0; r < BRICK_ROWS; r++)
    {
        for (int c = 0; c < MATRIX_COLS; c++)
        {
            if (bricks[r][c])
            {
                frame_buffer[r + BRICK_START_ROW][c] = 1; // <-- MODIFIED
            }
        }
    }
...
/**
 * @brief Draws the score bar on row 0.
 */
void draw_score(void)
{
    // Max score is 3 rows * 19 cols * 5 points/brick = 285
    const uint32_t max_score = BRICK_ROWS * MATRIX_COLS * 5;
    int bar_width = 0;

    if (score > 0)
    {
        // Calculate bar width, scale to 19 pixels (MATRIX_COLS - 1)
        bar_width = (int)(((float)score / (float)max_score) * (MATRIX_COLS - 1));
    }

    if (bar_width >= MATRIX_COLS) bar_width = MATRIX_COLS - 1;

    // Draw the bar on row 0
...
void render_frame(void)
{
    ...
    const uint32_t delay_per_led = 39;
