...
void turn_off_led(int anode_index, int cathode_index) {
    if (anode_index < 0 || cathode_index < 0) return;
...
/**
 * @brief Call this from main() to light all LEDs efficiently using POV.
 * This function loops forever and avoids overheating.
 */
void light_all_leds_efficient(void) {
    const uint32_t delay_per_led = 39; // 39us "on" time

    // Keep track of the previously lit LED
    int last_anode = -1;
    int last_cathode = -1;
    ...
    // Set ALL pins to input (Hi-Z) *ONCE* at the beginning.
    set_all_pins_input();
    ...
    while (1) {
        ...
        for (int i = 0; i < NUM_PINS; i++) {
            ...
            for (int j = 0; j < NUM_PINS; j++) {
                ...
                //    (This is now just 2 pin reconfigs)
                turn_off_led(last_anode, last_cathode);

                // 2. Turn ON the *current* LED
                //    (This is 2 pin reconfigs)
                light_led(i, j);

                // 3. Keep it on for a tiny amount of time
                DWT_Delay_us(delay_per_led);

                // 4. Remember which LED is on
                last_anode = i;
                last_cathode = j;
            }
        }
