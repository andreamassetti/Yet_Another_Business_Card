...
// The exact physical order of your Charlieplexing network (Pins 1-19)
const PinDef ALL_PINS[19] = {
    {GPIOB, 0}, {GPIOB, 1}, {GPIOB, 2},  {GPIOB, 3},  {GPIOB, 4},
    {GPIOB, 5}, {GPIOB, 6}, {GPIOB, 7},  {GPIOB, 8},  {GPIOB, 9},
    {GPIOB, 10},{GPIOB, 11},{GPIOB, 12}, {GPIOB, 13}, {GPIOB, 14},
    {GPIOB, 15},{GPIOC, 13},{GPIOC, 14}, {GPIOC, 15}
};
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if(system_active && (HAL_GetTick() - last_activity_time > 10000)){
        ...
        GPIOA->ODR = (1<<0);  // Keep PA0 Pull-Up active! (New wakeup button)
        GPIOB->ODR = 0; 
        GPIOC->ODR = 0; 
        GPIOD->ODR = 0;
...
void PrecomputeFrame(void) {
    ...
    // SAFETY FOR PA0: Default CRL for Port A must set Pin 0 to Input Pull-Up/Down (0x8)
    const uint32_t PORTA_CRL_SAFE = 0x44444448; 
    const uint32_t PORTA_ODR_SAFE = (1 << 0); // Pull-Up selected
    ...
    for(int row = 0; row < ROW_COUNT; row++) {
        ...
        for(int bitPlane = 0; bitPlane < COLOR_DEPTH; bitPlane++) {
            ...
            state->PortA_CRL = PORTA_CRL_SAFE; // PA0 Safety
            state->PortA_ODR = PORTA_ODR_SAFE; // PA0 Safety
...
static void MX_GPIO_Init(void)
{
  ...
  /* Configure PA0 for Wakeup Button */
